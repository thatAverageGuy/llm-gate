...
def _make_response(
    text: str = "hello", tokens: int = 10, provider: str = "openai"
) -> CompletionResponse:
    return CompletionResponse(
        ...
        usage=TokenUsage(
            prompt_tokens=5, completion_tokens=tokens - 5, total_tokens=tokens
        ),
    )
...
class TestBatch:
    ...
    def test_token_aggregation(self) -> None:
        ...
        responses = [
            _make_response(tokens=20),
            _make_response(tokens=30),
            _make_response(tokens=50),
        ]
        idx = 0
